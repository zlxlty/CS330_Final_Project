...
class XAxis(object):
    ...
    def draw(self, surface):
        ...
        for tick in self.ticks:
            ...
            pygame.draw.line(
                surface, SchedulingDisplayColors.TIME_TICK, p1, p2, lineWidth
            )
...
class YAxis(object):
    ...
    def draw(self, surface):
        ...
        for gridline in self.gridlines:
            ...
            pygame.draw.line(
                surface, SchedulingDisplayColors.TIME_BAR, p1, p2, lineWidth
            )
...
class IntervalRect(object):
    ...
    def build_rectangle(self, interval, startTime, endTime, numTasks, w, h, color):
        ...
        p1x = (
            int(
                float(interval.startTime - startTime)
                / totalTime
                * (w - BUFFER_LEFT - BUFFER_RIGHT)
            )
            + BUFFER_LEFT
        )
...
class ReleaseArrow(object):
    ...
    def build_arrow(
        self, releaseTime, taskId, numTasks, startTime, endTime, w, h, color
    ):
        plotHeight = h - BUFFER_TOP - BUFFER_BOTTOM
        plotBottom = h - BUFFER_BOTTOM
        taskHeight = plotHeight / numTasks
...
class DeadlineArrow(object):
    def __init__(self, deadlineTime, taskId, numTasks, startTime, endTime, w, h, color):
        self.build_arrow(
            deadlineTime, taskId, numTasks, startTime, endTime, w, h, color
        )

    def build_arrow(
        self, deadlineTime, taskId, numTasks, startTime, endTime, w, h, color
    ):
        plotHeight = h - BUFFER_TOP - BUFFER_BOTTOM
        plotBottom = h - BUFFER_BOTTOM
        taskHeight = plotHeight / numTasks
...
class CompletionHat(object):
    def __init__(
        self, completionTime, taskId, numTasks, startTime, endTime, w, h, color
    ):
        self.build_arrow(
            completionTime, taskId, numTasks, startTime, endTime, w, h, color
        )
    ...
    def build_arrow(
        ...
        pmx = (
            int(
                float(completionTime - startTime)
                / totalTime
                * (w - BUFFER_LEFT - BUFFER_RIGHT)
            )
            + BUFFER_LEFT
