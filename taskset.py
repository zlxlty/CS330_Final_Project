...
class TaskSet(object):
    ...
    def parseDataToTasks(self, data):
        ...
        self.tasks: Dict[int, Task] = taskSet
    ...
    def buildJobReleases(self, data):
        ...
        if TaskSetJsonKeys.KEY_RELEASETIMES in data:  # necessary for sporadic releases
            for jobRelease in data[TaskSetJsonKeys.KEY_RELEASETIMES]:
                releaseTime = float(
                    jobRelease[TaskSetJsonKeys.KEY_RELEASETIMES_JOBRELEASE]
                )
...
class Task(object):
    def __init__(self, taskDict):
        ...
        self.relativeDeadline = float(
            taskDict.get(
                TaskSetJsonKeys.KEY_TASK_DEADLINE,
    ...
    def spawnJob(self, releaseTime):
        ...
        if (
            ...
            and releaseTime < self.lastReleasedTime + self.period
        ):
    ...
    def __str__(self):
        return "task {0}: (Φ,T,C,D) = ({1}, {2}, {3}, {4})".format(
            self.id, self.offset, self.period, self.wcet, self.relativeDeadline
        )
...
#############################################################
# Handy helper classes (you can mostly ignore these)        #
#############################################################
